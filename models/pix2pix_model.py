...
from .losses import RL1Loss, PerceptualLoss
...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            if self.opt_content_loss == 'L2':
                ...
                self.criterionCtt.initialize(torch.nn.functional.l1_loss)
            elif self.opt_content_loss == 'Perceptual':
                self.criterionCtt = PerceptualLoss()
                self.criterionCtt.initialize(torch.nn.functional.mse_loss)
            else:
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        setattr(self, 'loss_G_'+self.opt_content_loss, self.criterionCtt(self.fake_B, self.real_B) * self.opt.lambda_L1)
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + getattr(self, 'loss_G_' + self.opt_content_loss)
